...
{{--                                     <div class="form-group">
                                        ...
                                        <div class="col-sm-7">
                                            <select class="form-control selectbox" name="NPKBawahan" id="NPKBawahan">
                                                <option value="">--- Pilih Bawahan ---</option>
                                                @foreach($data['bawahan'] as $bawahan)
                                                    <option value="{{ $bawahan->NPK }}" {{ (old('NPKBawahan') == $bawahan->NPK) ? 'selected="selected"' : '' }}>({{ $bawahan->NPK }}) {{ $bawahan->NamaKaryawan }}</option>
                                                @endforeach
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-3">
                                            <div class="input-group">
                                            <input type="number" step="any" min="0" lang="en" pattern="-?[0-9]+[\,.]*[0-9]+" class="form-control double" name="PersentaseKRA" id="PersentaseKRA" placeholder="Persentase KRA" value="{{ old('PersentaseKRA') }}">
                                                <span class="input-group-addon" id="basic-addon2">%</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    @for($x = 1; $x <= $data['target']; $x++)
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-5">
                                                <input type="hidden" value="" name="originalTarget{{ $targetparser->targetCount($data['target'])[$x-1] }}" id="originalTarget{{ $targetparser->targetCount($data['target'])[$x-1] }}">
                                                <input type="number" step="any" min="0" lang="en" pattern="-?[0-9]+[\,.]*[0-9]+" name="Target{{ $targetparser->targetCount($data['target'])[$x-1] }}" id="Target{{ $targetparser->targetCount($data['target'])[$x-1] }}" class="form-control double" placeholder="Target {{$x}}" value="{{ old('Target'.$targetparser->targetCount($data['target'])[$x-1]) }}">
                                            </div>
                                        </div>
                                    @endfor --}}
                                </div>
    ...
    <script>
            ...
            if (idkpi != '') {
                $.get(apiurl.replace(':iddetailrencana', idkpi), function(data) {
                    ...
                    for(i=1;i<=12;i++) {
                        ...
                        $("#target").val(data['Target' + i].toFixed(2));
                        ...
                        $("#max-target").html(data['Target' + i].toFixed(2));
        ...
        function setPersentaseKRA(){
            ...
            if(autoCompletePersentase == true){
                $("#form-table-penurunan").find(".PersentaseKRA").each(function(){
                    $(this).val(parseFloat(persentase / jumlah))
                });
            }else{
